...
import com.covidsemfome.module.report.process.EntradaReportProcess;
...
@TForm(name = "Pesquisar por entradas de produtos no Estocavel", showBreadcrumBar=true, editCssId="")
@TReportProcess(type=EntradaReportProcess.class, model = EstocavelReportModel.class)
@TPresenter(type = TDynaPresenter.class,
			...
			decorator = @TDecorator(type = TDataSetReportDecorator.class, 
									viewTitle="Relatório de Entrada de produtos no estoque"))
@TSecurity(	id="COVSEMFOME_ENTRADAREP_FORM", 
			appName = "#{app.name}", moduleName = "Administrativo", viewName = "Relatório de Entrada de produtos no estoque",
			allowedAccesses={TAuthorizationType.VIEW_ACCESS, TAuthorizationType.EXPORT, TAuthorizationType.SEARCH})
public class EntradaReportModelView extends TModelView<EstocavelReportModel>{
	...
	public EntradaReportModelView(EstocavelReportModel entidade) {
		super(entidade);
		entidade.setOrigem("Entrada");
	}
