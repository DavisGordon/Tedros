...
@TSecurity(	id="COVSEMFOME_CADCOZ_FORM", appName = "#{app.name}", moduleName = "Administrativo", viewName = "Local de produção",
...
public class CozinhaModelView extends TEntityModelView<Cozinha> {
	...
	@TTextReaderHtml(text="Cozinha", htmlTemplateForControlValue="<h2 id='"+THtmlConstant.ID+"' name='"+THtmlConstant.NAME
			+"' style='"+THtmlConstant.STYLE+"'>"+THtmlConstant.CONTENT+"</h2>",
			cssForControlValue="width:100%; padding:8px; background-color: "+TStyleParameter.PANEL_BACKGROUND_COLOR+";",
			cssForHtmlBox="", cssForContentValue="color:"+TStyleParameter.PANEL_TEXT_COLOR+";")
	@TFieldBox(alignment=Pos.CENTER_LEFT, node=@TNode(id="t-form",  parse = true))
	@TText(text="Dados da Cozinha", textAlignment=TextAlignment.LEFT, textStyle = TTextStyle.LARGE)
	private SimpleStringProperty header;
	
	@TReaderHtml
	@TLabel(text="#{label.name}")
	@TTextField(maxLength=60, required = true, textInputControl=@TTextInputControl(promptText="#{label.name}", parse = true), 
			node=@TNode(requestFocus=true, parse = true), control=@TControl(tooltip="#{label.name}", parse = true))
	...
	@TTextAreaField(control=@TControl(prefWidth=250, prefHeight=50, parse = true), wrapText=true, maxLength=200, prefRowCount=4)
	private SimpleStringProperty endereco;
	
	@TReaderHtml
	@TLabel(text="Telefone")
	@TMaskField(mask="(99) 999999999", textInputControl=@TTextInputControl(promptText="Telefone, celuar...", parse = true), required=true)
