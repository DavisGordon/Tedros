...
@TListViewPresenter(listViewMinWidth=350, listViewMaxWidth=350,
	paginator=@TPaginator(entityClass = Produto.class, serviceName = "IProdutoControllerRemote",
			...
			orderBy = {	@TOption(text = "#{label.codigo}", value = "codigo"), 
						@TOption(text = "#{label.name}", value = "nome")}),
	presenter=@TPresenter(decorator = @TDecorator(viewTitle="#{view.produtos}", buildImportButton=true),
	behavior=@TBehavior(importModelViewClass=ProdutoImportModelView.class, runNewActionAfterSave=true)))
@TSecurity(	id="SOLIDARITY_CADPROD_FORM", 
	appName = "#{app.name}", moduleName = "#{module.administrativo}", viewName = "#{view.produtos}",
...
public class ProdutoModelView extends TEntityModelView<Produto> {
	...
	@TTextReaderHtml(text="#{label.produto}", 
			htmlTemplateForControlValue="<h2 id='"+THtmlConstant.ID+"' name='"+THtmlConstant.NAME+"' style='"+THtmlConstant.STYLE+"'>"+THtmlConstant.CONTENT+"</h2>",
			cssForControlValue="width:100%; padding:8px; background-color: "+TStyleParameter.PANEL_BACKGROUND_COLOR+";",
			cssForHtmlBox="", cssForContentValue="color:"+TStyleParameter.PANEL_TEXT_COLOR+";")
	@TFieldBox(alignment=Pos.CENTER_LEFT, node=@TNode(id="t-form", parse = true))
	@TText(text="#{text.produto}", textAlignment=TextAlignment.LEFT, 
	...
	@TReaderHtml
	@TLabel(text="#{label.marca}")
	@TTextField(maxLength=20,
	textInputControl=@TTextInputControl(promptText="#{prompt.marca}", parse = true), 
				control=@TControl(tooltip="#{tooltip.marca}", parse = true))
	...
	@TReaderHtml
	@TLabel(text="#{label.medida}")
	@TTextField(maxLength=10, 
			textInputControl=@TTextInputControl(promptText="#{label.medida}", parse = true), 
						control=@TControl(tooltip="#{tooltip.medida}", parse = true))
