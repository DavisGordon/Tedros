...
@TSelectionModalPresenter(
		...
		tableView=@TTableView(editable=true, 
			columns = { @TTableColumn(cellValue="descricao", text = "#{label.descricao}", prefWidth=20, resizable=true), 
						@TTableColumn(cellValue="tipoPessoa", text = "#{label.tipo}", resizable=true), 
...
public class TipoAjudaFindModelView extends TEntityModelView<TipoAjuda> {
	...
	textInputControl=@TTextInputControl(promptText="#{label.descricao}", parse = true), 
