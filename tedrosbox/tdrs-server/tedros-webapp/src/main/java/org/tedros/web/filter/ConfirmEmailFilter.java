 ...
 *
 */
@WebFilter(filterName = "ConfirmEmailFilter",
urlPatterns = {"/pt/confirmemail.html", "/en/confirmemail.html" })
public class ConfirmEmailFilter implements Filter {
	...
	public void doFilter(ServletRequest req, ServletResponse resp, FilterChain chain)
	    ...
	    if(StringUtils.isNotBlank(key) && StringUtils.isNotBlank(lang)){
			...
			try {
				TResult<Boolean> res = serv.confirmEmail(appBean.getToken(), locale, key.trim());
				if(res.getState().equals(TState.SUCCESS)){
					chain.doFilter(req, resp);
				}else if(res.getState().equals(TState.WARNING)){
