 ...
 */
@TRemoteSecurity
@Stateless(name="ISiteEquipeController")
@TransactionAttribute(value = TransactionAttributeType.NOT_SUPPORTED)
public class SiteEquipeController extends TSecureEjbController<SiteEquipe> implements ISiteEquipeController, ITSecurity {
	
	@EJB
	private TStatelessService<SiteEquipe> serv;
	...
	@Override
	public ITEjbService<SiteEquipe> getService() {
		return serv;
	}
	...
	@Override
	public TResult<SiteEquipe> save(TAccessToken token, SiteEquipe e) {
		
		List<TFileEntity> filesToRem = new ArrayList<>();
		if(!e.isNew()) {
			try {
				SiteEquipe  x = serv.findById(e);
		...
		TResult<SiteEquipe> res = super.save(token, e);
