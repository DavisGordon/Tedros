 ...
 *
 */
@WebFilter(filterName = "TedrosWebFilter",
urlPatterns = {"/tdrs/*", "/api/tdrs/*"})
public class TedrosWebFilter implements Filter {
	    	...
	    	try {
				...
				if(res.getResult().equals(EnumResult.SUCESS)){
					if(res.getValue()){
						...
						if(u.getPessoa().isVoluntarioEstrategico()) {
	...
	private void setResponse(ServletResponse resp, HttpServletRequest httpRequest, int status, String targetPage) throws IOException {
		if(httpRequest.getRequestURL().toString().contains("api/tdrs"))
