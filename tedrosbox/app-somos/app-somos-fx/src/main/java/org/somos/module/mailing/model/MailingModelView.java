...
import org.somos.module.acao.table.VoluntarioTableView;
import org.somos.module.mailing.behavior.MailingBehavior;
import org.somos.module.mailing.behavior.MailingSaveAction;
import org.somos.module.mailing.decorator.MailingDecorator;
import org.somos.module.mailing.form.EmailTemplateForm;
 ...
 *
 */
@TForm(name = "Ação", showBreadcrumBar=false, form=EmailTemplateForm.class)
...
@TListViewPresenter(listViewMinWidth=350, listViewMaxWidth=350,
	...
	presenter=@TPresenter(behavior = @TBehavior(type = MailingBehavior.class, 
		...
		decorator = @TDecorator(type = MailingDecorator.class, 
			viewTitle="Ação", listTitle="Acão", saveButtonText="Enviar email")))
@TSecurity(	id="SOMOS_MAIL_FORM", 
	appName = "#{somos.name}", moduleName = "Gerenciar Campanha", viewName = "Mailing de Ação",
...
public class MailingModelView extends TEntityModelView<Mailing> {
	...
	public MailingModelView(Mailing entity) {
		super(entity);
		tituloEmail.setValue("[Social Movement] "+getModel().getTitulo());
